...
const pat = fs
	.readFileSync(path.join(__dirname, "..", "pat"))
	.toString()
	.trim();
...
app.post("/login", async (req, res) => {
	...
	try {
		const response = await fetch(
			`https://www.recurse.com/api/v1/profiles?query=${email}`,
			{
				method: "GET",
				headers: {
			...
			if (Array.isArray(rc_users)) {
				if (rc_users.length !== 0) {
					...
					res.status(200).send({
						...
						hasPAT: user.PAT === undefined ? false : true,
					});
				} else {
				...
				throw new Error(
					`Recurse Center API returned unexpected results: ${rc_users}`,
				);
			}
		} else {
...
app.post("/macaddress", async (req, res) => {
	...
	if (!macAddressRegex.test(macAddress)) {
		...
		res
			.status(500)
			.send(
				"This MAC Address has already been registered for a different user.",
			);
		return;
	}

	// Save the MAC address to the database
	db.data.macAddresses[macAddress] = {
		user_id: rc_user.id,
		last_visit_created_on: undefined,
	};
...
function logPacketInfo(packet) {
  ...
  ${nbytes} bytes, truncated? ${trunc}`);
	console.log(
		`  Sender MAC Address: ${packet.info.sendermac}${packet.info.sendermac === "c8:2a:dd:b0:7d:9d" ? " - MY PHONE!!!" : ""}`,
	);
	...
	if (registeredMacAddress !== undefined) {
		...
		if (user.last_visit_created_on !== today) {
			if (user.PAT !== undefined) {
				...
				try {
					const response = await fetch(
						`https://www.recurse.com/api/v1/hub_visits/${user_id}/${today}`,
						{
							method: "PATCH",
							headers: {
